"""透明代理核心：路由匹配、httpx 流式/缓冲转发、日志持久化与 WebSocket 广播。"""
...
from transparent_proxy_gateway.proxy.client import get_http_client
from transparent_proxy_gateway.proxy.connectivity import format_upstream_error
from transparent_proxy_gateway.proxy.headers import BODYLESS_METHODS, filter_hop_by_hop, prepare_outgoing_headers
from transparent_proxy_gateway.proxy.router import build_target_from_request, match_route
...
def get_client_ip(request) -> str | None:
    """从请求中解析客户端 IP（优先 X-Forwarded-For）。"""
...
async def _forward_httpx_stream(
    *,
    ...
    """主路径：httpx 流式发送 + Django StreamingHttpResponse。"""
    ...
    from transparent_proxy_gateway.proxy.response_rewrite import apply_response_rewrites, set_response_headers

    from transparent_proxy_gateway.proxy.response_rewrite import is_redirect_status
...
async def _forward_buffered_fallback(
    *,
    ...
    """
    缓冲模式回退：整包读取上游响应（不适合大文件下载）。

    由 ``PROXY_FORWARD_MODE=buffered`` 启用。
    """
    body = await _read_request_body(request)
    outgoing_headers = prepare_outgoing_headers(request, target_url, route=route)
    cookies = dict(request.COOKIES)
...
async def forward_request(request) -> StreamingHttpResponse | HttpResponseNotFound:
    """
    透明异步反向代理入口：前缀匹配后转发到上游。

    默认 httpx 流式；可通过 ``PROXY_FORWARD_MODE`` 切换为缓冲模式。
    """
